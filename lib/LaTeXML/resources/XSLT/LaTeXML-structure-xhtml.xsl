...
<xsl:stylesheet
  ...
  <xsl:template match="ltx:document | ltx:part | ltx:chapter
    ...
    <xsl:text>&#x0A;</xsl:text>
    <xsl:element name="{f:if($USE_HTML5,f:if(local-name(.) = 'document','article','section'),'div')}" namespace="{$html_ns}">
...
<!--          <xsl:value-of select="f:seclev-aux('document')+number(boolean(//ltx:document/ltx:title))"/>-->
        ...
        <xsl:when test="$name = 'appendix' or $name = 'index'
          ...
          <xsl:value-of select="f:if(//ltx:chapter,f:seclev-aux('chapter'),f:seclev-aux('section'))"/>
        </xsl:when>
  ...
  <xsl:template match="ltx:indexlist" mode="twocolumn">
    ...
    <xsl:param name="miditem" select="count($lines[position() &lt; $halflines]/ancestor::ltx:indexentry[parent::ltx:indexlist[parent::ltx:index]]) + 1"/>
